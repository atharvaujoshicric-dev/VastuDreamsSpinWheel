...
# Prize data matching the image clockwise starting from the TOP segment
prizes = [
    {"label": "BETTER LUCK NEXT TIME", "icon": "❌"}, # Top (White)
    {"label": "1.5 LACS OFF", "icon": "💵"},         # Top-Right (Pink)
    {"label": "2 LACS OFF", "icon": "💎"},           # Bottom-Right (Orange)
...
# --- CSS FIXES FOR PERFECT CENTERING ---
st.markdown("""
    <style>
    ...
    /* Center the component container */
    iframe {
        display: block;
        margin: 0 auto;
    }
...
# =====================
    ...
    wheel_html = f"""
    <div id="wrapper" style="display: flex; flex-direction: column; align-items: center; width: 100%; height: 100%;">
        <div id="pointer" style="width: 0; height: 0; border-left: 15px solid transparent; border-right: 15px solid transparent; border-top: 25px solid #C9A84C; z-index: 100; margin-bottom: -10px; position: relative;"></div>

        <div id="wheel-container" style="position: relative; width: 340px; height: 340px; border-radius: 50%; border: 8px solid #C9A84C; background: #000; box-shadow: 0 0 30px rgba(201,168,76,0.3); display: flex; align-items: center; justify-content: center; overflow: hidden;">
            <img id="wheel-img" src="data:image/png;base64,{wheel_base64}" 
                 style="width: 101%; height: 101%; object-fit: cover; transition: transform 6s cubic-bezier(0.1, 0, 0, 1); transform: rotate(0deg);">
        </div>
    ...
    btn.addEventListener('click', () => {{
        ...
        setTimeout(() => {{
            ...
            // netRotation is the final angle of the image.
            // We subtract 30 degrees to account for the pointer pointing to the CENTER of the first slice.
